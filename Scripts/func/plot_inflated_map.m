function [hfig,ax] = plot_inflated_map(data,minv,maxv,cmap,options)
% IN
% - data = vector [2447,1]
...
if ~strcmp(options.subplot_dist,'aligned')
...
elseif strcmp(options.subplot_dist,'aligned')

    % I reduce the size (3,4 = width and height), so that they all fit into
    % one linem put all y (2) pos at .1 and just change x (1) pos by incrementing
    % by sligthly more than width
    ...
    % mask
    pos=get(ax(5),'Position');
    pos(1)= newx + .11/1.5; % .11 is the original x distance between ax3 and 5
    pos(2) = newy + .077/1.5; % .077 is the original y distance between ax3 and 5
    pos(3) = pos(3)/1.5;
    pos(4) = pos(4)/1.5;
    set(ax(5),'Position',pos);
    clear newx nwey
